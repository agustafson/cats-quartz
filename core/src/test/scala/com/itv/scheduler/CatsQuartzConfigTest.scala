...
class CatsQuartzConfigTest extends AnyFlatSpec with Matchers with ScalaCheckDrivenPropertyChecks {
  behavior of "CatsQuartzConfig"
  ...
  it should "load config correctly" in {
    forAll(
      ...
      val quartzConfig = CatsQuartzConfig(
        JobStoreConfig(
